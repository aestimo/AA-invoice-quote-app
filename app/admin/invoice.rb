...
ActiveAdmin.register Invoice do
  ...
  permit_params :client_id, :items_attributes, :code, :status, :due_date, :tax, :discount, :terms, :notes
    ...
    show :title => :code do
      ...
      panel "Items" do
        table_for invoice.items do |t|
          t.column("Qty.") { |item| number_with_delimiter item.qty }
          t.column("Description") { |item| item.description }
          t.column("Unit price") { |item| number_to_currency item.unit_price }
    ...
    form do |f|
      f.inputs "Client" do
        f.input :client, :collection => Client.pluck(:name)
      end

      f.inputs "Items" do
        f.has_many :items do |i|
          i.input :_destroy, :as => :boolean, :label => "Delete this item" unless i.object.id.nil?
          i.input :name, :collection => Item.order('name ASC')
          i.input :qty, :hint => "The total inventory quantity for this item is currently : #{Invoice.current_qty}"
          i.input :unit_price
        end
      end
